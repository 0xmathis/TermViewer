...
pub struct McuDrawer;

impl McuDrawer {
    fn level1<T: Read>(image: BMP<T>, terminal_width: usize, terminal_height: usize) -> Result<()> {
    ...
    fn level2<T: Read>(image: BMP<T>, terminal_width: usize, terminal_height: usize) -> Result<()> {
        let image_height: usize = image.height() as usize;
        let image_width: usize = image.width() as usize;
        let mcus: &Vec<MCU> = image.mcus();
        let mcu_width: usize = (image_width as usize + 7) / 8;
        ...
        loop {
            let (r, g, b): (u8, u8, u8) = Self::average(mcus, mcu_width, image_width, image_height, image_row, image_column, step);
            output.push_str(&Self::background(r, g, b));
            output.push_str(&"  ");
    ...
    pub fn draw<T: Read>(&self, image: BMP<T>, scaling_level: ScalingLevel) -> Result<()> {
        Self::clean();
        Self::hide_cursor();
